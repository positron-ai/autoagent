...
class AresIngestCliTest(unittest.TestCase):
  ...
  def test_print_json_keeps_stdout_parseable(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      run_dir = root / "run"
      stdout = io.StringIO()
      ...
      self.assertEqual(state["status"], "blocked_no_refiner")
      self.assertEqual(
        state["reward"]["first_failed_gate"],
        "hf_cpu_oracle",
      )
      self.assertIn("ares-ingest-agent status=blocked_no_refiner", stderr.getvalue())
      self.assertNotIn("ares-ingest-agent status=", stdout.getvalue())
  ...
  def test_evidence_gate_preserves_gate_specific_skill_context(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      evidence_skills = [skill for skill in skills if skill["name"] == "ares-evidence"]
  ...
  def test_model_spec_gate_selects_model_port_skill_context(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      model_skill = next(skill for skill in skills if skill.get("gate") == "model_spec")
      ...
      self.assertIn("clone any missing official upstream repos", model_skill["why"])
      self.assertIn("https://github.com/vllm-project/vllm.git", model_skill["why"])
      self.assertIn(
  ...
  def test_model_spec_handoff_notes_are_preserved(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      run_dir = root / "run"
      run_dir.mkdir()
      ...
      (run_dir / "model_spec.json").write_text(
        json.dumps(
          {
            "handoff_notes_markdown": (
              "## Durable Resume State\n\n"
              "- Current first failing gate: `one_token_logits`.\n"
              "- Run the FPGA helper on a card host."
            )
          }
        )
      )
      ...
      write_handoff(cfg, reward, state={"history": []})
      handoff = (run_dir / "handoff.md").read_text()
      self.assertIn("## Durable Resume State", handoff)
      self.assertIn("Run the FPGA helper on a card host", handoff)
      self.assertLess(
        handoff.index("## Durable Resume State"),
        handoff.index("## Next Action"),
      )

  def test_model_spec_handoff_notes_path_is_preserved(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      run_dir = root / "run"
      notes = run_dir / "artifacts" / "handoff-notes.md"
      ...
      (run_dir / "model_spec.json").write_text(
        json.dumps({"handoff_notes_path": "artifacts/handoff-notes.md"})
      )
      reward = {
        "score": 0.49,
        "alpha_execution": 0.8,
  ...
  def test_cpp_tvd_selects_comparison_evidence_context(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      cpp_skill = next(skill for skill in skills if skill.get("gate") == "cpp_tvd")
  ...
  def test_mmlu_pro_gate_selects_mmlu_skill_context(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      mmlu_skill = next(skill for skill in skills if skill.get("gate") == "mmlu_pro")
  ...
  def test_refiner_loop_writes_prompt_log_and_history(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      self.assertIn("Work only the first failing gate: `hf_cpu_oracle`", prompt_text)
  ...
  def test_backend_profile_writes_command_wrapper_plan(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      self.assertTrue(all(wrapper["missing_inputs"] for wrapper in plan["wrappers"]))
