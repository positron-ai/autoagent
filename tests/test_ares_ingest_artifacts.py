...
def one_token_artifacts(root: Path) -> dict[str, dict[str, object]]:
  hf = root / "hf-dense.jsonl"
  ares = root / "ares-dense.jsonl"
  runtime = root / "runares"
  ares_plan = root / "plan.json"
  target_plan = root / "target.json"
  hf.write_text('{"logits":[0.0,1.0]}\n')
  ares.write_text('{"logits":[0.0,1.0]}\n')
  runtime.write_bytes(b"synthetic-runares")
  ares_plan.write_text('{"schema_version":2}\n')
  target_plan.write_text('{"schema_version":1}\n')
  return {
    "hf_cpu_dense_logits_jsonl": file_record(hf, rows=1),
    "ares_dense_logits_jsonl": file_record(ares, rows=1),
    "runtime_binary": file_record(runtime),
    "ares_plan": file_record(ares_plan),
    "target_plan": file_record(target_plan),
  }


def eight_token_artifacts(
  root: Path,
  reference: Path,
  candidate: Path,
) -> dict[str, dict[str, object]]:
  hf = root / "hf-eight-dense.jsonl"
  ares = root / "ares-eight-dense.jsonl"
  runtime = root / "eight-runares"
  ares_plan = root / "eight-plan.json"
  target_plan = root / "eight-target.json"
  hf.write_text('{"logits":[0.0,1.0]}\n')
  ares.write_text('{"logits":[0.0,1.0]}\n')
  runtime.write_bytes(b"synthetic-eight-runares")
  ares_plan.write_text('{"schema_version":2}\n')
  target_plan.write_text('{"schema_version":1}\n')
  return {
    "reference_tokens": file_record(reference),
    "candidate_tokens": file_record(candidate),
    "hf_cpu_dense_logits_jsonl": file_record(hf, rows=1),
    "ares_dense_logits_jsonl": file_record(ares, rows=1),
    "runtime_binary": file_record(runtime),
    "ares_plan": file_record(ares_plan),
...
def compact_eight_token_artifacts(
  root: Path,
  reference: Path,
  candidate: Path,
) -> dict[str, dict[str, object]]:
  hf_report = root / "hf-reference-report.json"
  ares_token_lines = root / "runares.log"
  runtime = root / "compact-runares"
  ares_plan = root / "compact-plan.json"
  target_plan = root / "compact-target.json"
  hf_report.write_text(
    json.dumps(
      {
        "schema": "ares.hf_export.reference_tokens.v1",
        "generated_ids": list(range(8)),
        "generated_count": 8,
      },
      sort_keys=True,
    )
  ...
  ares_token_lines.write_text(
    "".join(
      f"ARES_EMIT_TOKEN_ID position={position} id={token} request=1\n"
      for position, token in enumerate(range(8))
    )
  )
  runtime.write_bytes(b"synthetic-compact-runares")
  ares_plan.write_text('{"schema_version":2}\n')
  target_plan.write_text('{"schema_version":1}\n')
  return {
    "reference_tokens": file_record(reference),
    "candidate_tokens": file_record(candidate),
    "hf_cpu_reference_report": file_record(hf_report),
    "ares_token_lines": file_record(ares_token_lines, rows=8),
    "runtime_binary": file_record(runtime),
    "ares_plan": file_record(ares_plan),
    "target_plan": file_record(target_plan),
  }
...
class AresIngestArtifactTest(unittest.TestCase):
  ...
  def test_one_token_logits_gate_rejects_missing_artifacts(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      self.assertFalse(gate["passed"])
      self.assertIn("one-token artifacts", " ".join(gate["errors"]))

  def test_one_token_logits_gate_rejects_missing_model_provenance(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      path = root / "one-token.json"
      path.write_text(
      ...
      self.assertFalse(gate["passed"])
      self.assertIn("model_provenance must be an object", " ".join(gate["errors"]))

  def test_one_token_logits_gate_rejects_missing_runtime_artifact(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      artifacts = one_token_artifacts(root)
      del artifacts["runtime_binary"]
      path = root / "one-token.json"
      path.write_text(
        json.dumps(
          {
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "one-token artifacts.runtime_binary must be an object",
        " ".join(gate["errors"]),
      )

  def test_one_token_logits_gate_rejects_diagnostic_class(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      path = root / "one-token.json"
      path.write_text(
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "evidence_class must be system_under_test or promotion",
        " ".join(gate["errors"]),
      )

  def test_one_token_logits_gate_rejects_preflight_report_schema(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      path = root / "one-token-preflight.json"
      path.write_text(
        json.dumps(
          {
            "schema": "ares.runtime.one_token_logits.preflight_report.v1",
            "evidence_class": "diagnostic",
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "preflight reports are diagnostic and cannot close one_token_logits",
        " ".join(gate["errors"]),
      )

  def test_one_token_logits_gate_rejects_stale_artifact_digest(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      artifacts = one_token_artifacts(root)
      artifacts["ares_dense_logits_jsonl"]["sha256"] = SHA_A
      path = root / "one-token.json"
      path.write_text(
        json.dumps(
          {
  ...
  def test_token_agreement_gate_accepts_eight_token_evidence(self) -> None:
    with TemporaryDirectory() as tmp:
      ...
      evidence = build_greedy_token_evidence(
        {
          "score": 1.0,
          "exact_match": True,
          "exact_fraction": 1.0,
      ...
      evidence["artifacts"] = eight_token_artifacts(root, reference, candidate)
  ...
  def test_token_agreement_gate_accepts_compact_token_line_evidence(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
      ...
      evidence["model_provenance"] = model_provenance()
      evidence["artifacts"] = compact_eight_token_artifacts(
        root,
        reference,
        candidate,
      )
      ...
      self.assertTrue(gate["passed"])
      self.assertEqual(gate["artifact_validator"], "eight_token_greedy")
      self.assertIn("ares_token_lines", gate["detail"]["artifact_keys"])
      self.assertIn("hf_cpu_reference_report", gate["detail"]["artifact_keys"])

  def test_token_agreement_gate_rejects_missing_artifacts(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
      ...
      self.assertFalse(gate["passed"])
      self.assertIn("eight-token artifacts", " ".join(gate["errors"]))

  def test_token_agreement_gate_rejects_missing_model_provenance(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
  ...
  def test_token_agreement_gate_rejects_missing_runtime_artifact(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
      ...
      reference.write_text(json.dumps(reference_payload))
      candidate.write_text(json.dumps(candidate_payload))
      artifacts = eight_token_artifacts(root, reference, candidate)
      artifacts.pop("ares_dense_logits_jsonl")
      evidence = build_greedy_token_evidence(
        {
          "score": 1.0,
          "exact_match": True,
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "ares_dense_logits_jsonl or ares_token_lines",
        " ".join(gate["errors"]),
      )

  def test_token_agreement_gate_rejects_missing_hf_reference_artifact(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
      ...
      candidate.write_text(json.dumps(candidate_payload))
      artifacts = eight_token_artifacts(root, reference, candidate)
      artifacts.pop("hf_cpu_dense_logits_jsonl")
      evidence = build_greedy_token_evidence(
        {
          "score": 1.0,
          "exact_match": True,
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "hf_cpu_dense_logits_jsonl, hf_cpu_oracle_jsonl, or hf_cpu_reference_report",
        " ".join(gate["errors"]),
      )

  def test_token_agreement_gate_accepts_hf_oracle_reference_artifact(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
      oracle = root / "hf-eight-oracle.jsonl"
      path = root / "tokens.json"
      reference_payload = {"generated_token_ids": list(range(8))}
      candidate_payload = {"generated_token_ids": list(range(8))}
      reference.write_text(json.dumps(reference_payload))
      candidate.write_text(json.dumps(candidate_payload))
      oracle.write_text('{"generation":{"generated_token_ids":[0,1,2,3,4,5,6,7]}}\n')
      artifacts = eight_token_artifacts(root, reference, candidate)
      artifacts.pop("hf_cpu_dense_logits_jsonl")
      artifacts["hf_cpu_oracle_jsonl"] = file_record(oracle, rows=1)
      evidence = build_greedy_token_evidence(
        {
          "score": 1.0,
          "exact_match": True,
  ...
  def test_token_agreement_gate_rejects_diagnostic_class(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      reference = root / "reference.json"
      candidate = root / "candidate.json"
  ...
  def test_token_agreement_gate_rejects_preflight_report_schema(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      path = root / "eight-token-preflight.json"
      path.write_text(
        json.dumps(
          {
            "schema": "ares.runtime.eight_token_greedy.preflight_report.v1",
            "evidence_class": "diagnostic",
            "status": "preflight_passed",
          }
        )
      ...
      self.assertFalse(gate["passed"])
      self.assertIn(
        "preflight reports are diagnostic and cannot close eight_token_greedy",
        " ".join(gate["errors"]),
      )
  ...
  def test_mmlu_pro_gate_accepts_project_scoped_uv_command(self) -> None:
    with TemporaryDirectory() as tmp:
      root = Path(tmp)
      path = root / "mmlu-pro.json"
      path.write_text(
        json.dumps(
          mmlu_pro_payload(
            ...
            command=(
              ...
              "uv run --project /repo/third_party/systems_test "
              "--locked mmlu_pro"
            ),
          )
        )
        ...
        json.dumps(
          mmlu_pro_payload(
            ...
            command=(
              "OPENAI_HOST=http://127.0.0.1:50183/v1 "
              "SKIP_PROVISION=1 MMLU_MODEL=synthetic/model "
              "uv run --project /repo/third_party/systems_test pytest"
            ),
          )
        )
      )
