...
def workflow_skill_lines(skills: list[Mapping[str, Any]]) -> list[str]:
  ...
  for skill in skills:
    ...
    if isinstance(allowed, list) and allowed:
      lines.append("  Allowed scope: " + ", ".join(f"`{item}`" for item in allowed))
    commands = skill.get("verification_commands", [])
    if isinstance(commands, list) and commands:
      lines.append("  Verification: " + ", ".join(f"`{item}`" for item in commands))
  return lines
  ...
  if path.suffix == ".jsonl":
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
      ...
      try:
        ...
        raise AresIngestError(f"invalid JSONL in {path}:{lineno}: {exc}") from exc
    return rows
  try:
...
def handoff_notes_markdown(cfg: AresIngestConfig) -> str:
  if not cfg.model_spec_path.exists():
    return ""
  try:
    spec = load_json(cfg.model_spec_path)
  except AresIngestError:
    return ""
  notes_path = spec.get("handoff_notes_path")
  if isinstance(notes_path, str) and notes_path:
    try:
      return resolve_run_path(notes_path, cfg).read_text().strip()
    except OSError:
      return ""
  notes = spec.get("handoff_notes_markdown")
...
def write_handoff(
  ...
  *,
  ...
  handoff_notes = handoff_notes_markdown(cfg)
  handoff_notes_block = f"\n{handoff_notes}\n" if handoff_notes else ""
...
def evaluate_run(cfg: AresIngestConfig) -> tuple[dict[str, Any], dict[str, Any]]:
  ...
  if token_spec := spec.get("token_comparison"):
    ...
    token_payload = build_greedy_token_evidence(
      ...
      expected_generated_tokens=int(token_spec.get("expected_generated_tokens", 8)),
    )
